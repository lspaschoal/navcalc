...
<title>Admin > Aeronaves > Editar Aeródromo</title>
<style>
.card{
margin: 0 auto;
...
<div style="text-align: center; margin: 100px;"><span class="titulo">Editar Aeródromo</span></div>
...
<form action="servletUpdateAerodromo" method="post" onsubmit="return confirm('Tem certeza de que deseja salvar essas alterações?')">
<% Aerodromo aerodromo = (Aerodromo) request.getAttribute("aerodromo");%>
<input type="hidden" name="idAerodromo" value="<%= aerodromo.getId() %>">
<table class="card">
<tr><td>Código ICAO:</td><td><input type="text" name="icao" value="<%= aerodromo.getIcao() %>" placeholder="Digite o ICAO do tipo"><label class="erro"><%= request.getAttribute("erroIcao") %></label></td></tr>
<tr><td>Nome:</td><td><input type="text" name="nome" value="<%= aerodromo.getNome() %>" placeholder="Digite nome do aeródromo"><label class="erro"><%= request.getAttribute("erroNome") %></label></td></tr>
<tr><td>Elevação:</td><td><input type="number" name="elevacao" value="<%= aerodromo.getElevacao() %>" placeholder="pés"><label class="erro"><%= request.getAttribute("erroElevacao") %></label></td></tr>
