	...
	public Planejamento gerarPlanejamento() {
		...
		for(String s: coordenadas) {
			try {
				...
				fixos.add(new Fixo(latgraus, latmin, latseg, lathemisferio, longraus, lonmin, lonseg,
						lonhemisferio));
			}
		}
		plan.setRota(fixos);
		...
		if(plan.getRota() != null && !plan.getRota().isEmpty()) {
			...
			for(int i = 0; i < plan.getRota().size() - 1; i++) {
				if(plan.getRota().get(i).getNome() != null) {
					...
					String latitude = plan.getRota().get(i).latitudeToString().replace("°", "").replace("'", "").replace("\"", "");
					String longitude = plan.getRota().get(i).longitudeToString().replace("°", "").replace("'", "").replace("\"", "");
			...
			if(plan.getRota().get(plan.getRota().size() - 1).getNome() != null) {
				...
				String latitude = plan.getRota().get(plan.getRota().size() - 1).latitudeToString().replace("°", "").replace("'", "").replace("\"", "");
				String longitude = plan.getRota().get(plan.getRota().size() - 1).longitudeToString().replace("°", "").replace("'", "").replace("\"", "");
				this.rota += latitude + longitude;
			}
